...
# The maximum number of lines (default 500) contained in the history file (file
# location defined with HISTFILE default ~/.bash_history). This parameter isn't
# needed here as we don't change HISTFILE location.
#HISTFILESIZE=2000
...
if type source-highlight-esc.sh >/dev/null 2>&1; then
...
#{{{ Compressed files extraction management
#-------------------------------------------------------------------------------
extract() {
    local c e i

    (($#)) || echo "[${RED}+${OFF}] No file specified. Quitting..." || return
    ...
    for i; do
        ...
        case $i in
            ...
            *.xz)  c='unxz';;
            *.zip) c='unzip';;
            *)     echo "[${RED}+${OFF}] $0: unrecognized file extension: \`$i'" >&2
                   continue;;
        esac
